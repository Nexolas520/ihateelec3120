  ...
  switch (flags) {
    case ACK_FLAG_MASK: {
      ...
      if (after(ack, sock->window.last_ack_received)) {
        ...
        // Slide the window
        sock->window.last_byte_acked = ack;
      }
      break;
    }
    ...
    default: {
      if (get_payload_len(pkt) > 0) {
        ...
        debug_printf("Received data packet %d %d\n", seq, seq + get_payload_len(pkt));

        sock->window.advertised_window = get_advertised_window(hdr);

        // Check if the packet is within the expected sequence range
  ...
  if (buf_len > 0) {
    while (buf_len != 0) {
      ...
      slot.msg = create_packet(
          ...
          0, MAX(MAX_NETWORK_BUFFER - (uint32_t)sock->received_len, MSS), 0, NULL,
...
void transmit_send_window(foggy_socket_t *sock) {
  for (auto &slot : sock->send_window) {
    if (!slot.is_sent) {
      ...
      debug_printf("Sending packet %d %d\n", get_seq(hdr), get_seq(hdr) + get_payload_len(slot.msg));
      slot.is_sent = 1;
